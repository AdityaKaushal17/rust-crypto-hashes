...
impl Sha256 {
    ...
    fn transform(&mut self) {
        ...
        for i in 16..64 {
            let s0 = Self::rotr(m[i - 15], 7) ^ Self::rotr(m[i - 15], 18) ^ (m[i - 15] >> 3);
            let s1 = Self::rotr(m[i - 2], 17) ^ Self::rotr(m[i - 2], 19) ^ (m[i - 2] >> 10);
    ...
    pub fn finalize(mut self) -> [u8; 32] {
        let bitlen = self.bitlen + (self.datalen as u64) * 8;
        ...
        for i in 0..8 {
            self.data[63 - i] = (bitlen >> (i * 8)) as u8;
        }
